...
char get_char (void)
{
	/** UARTの受信データを取得する処理を実装する。 getcharとかそのまま使える？ */
	#if 0
...
int execute (int argc, const char * const * argv)
{
	...
	while (i < argc) {
		if (strcmp (argv[i], _CMD_HELP) == 0) {
			...
			int retval = monitor_execute (argc, argv);
			if ( retval == 0) {
				// if command not found in monitor, print error message
				print ("command: '");
				print ((char*)argv[i]);
				print ("' Not found.\n\r");
			} else {
...
/* アプリケーションコマンドをこの部分に追加する。 argcが引数の個数。 argvが実際の引数文字列。 */
#define _CMD_NULL "null"
int monitor_execute (int argc, const char * const * argv) {
	if (strcmp (argv[0], _CMD_NULL) == 0) {
		print ("nullpo!\n");
		return 1;
	} else {
		return 0;
